...
async function executeAddComment(args: z.infer<typeof parameters>, log: any) {
...
function addCommentTool(server: FastMCP, name: 'addComment' | 'createComment') {
  server.addTool({
    name,
    description:
      'Adds a Google Docs comment to the document at the specified text range. Use listComments to retrieve the comment ID after creation. Note: programmatically created comments appear in the comments panel but may not show as anchored highlights in the document UI.',
    parameters,
    execute: async (args, { log }) => executeAddComment(args, log),
  });
}

export function register(server: FastMCP) {
  addCommentTool(server, 'addComment');
  addCommentTool(server, 'createComment');
}
